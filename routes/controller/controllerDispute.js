...
const {
  ...
  sellerRespondSchema,
  adminDecisionSchema,
} = require("../services/validations/disputeValidation");
...
async function logHistory(
  { disputeId, event, title, note, actor },
  session = null
) {
  return DisputeHistory.create([{ disputeId, event, title, note, actor }], {
    session,
  });
}
    ...
    const disputeNotifications = [
      {
        ...
        message: `A buyer has raised a dispute for order ${order._id
          .toString()
          .slice(-6)}. Reason: ${disputeType}`,
  ...
  try {
    ...
    dispute.adminReview = {
      ...
      disputeAmountPercent:
        decision === "BUYER" ? disputeAmountPercent || 0 : 0,
      resolvedAt: new Date(),
    };
    ...
    const adminDecisionNotifications = [
      {
        ...
        message: `Your dispute has been resolved by admin. Decision: ${
          decision === "BUYER"
            ? "In your favour"
            : decision === "SELLER"
            ? "In favour of seller"
      ...
      },
      {
        ...
        type: NOTIFICATION_TYPES.DISPUTE,
        title: "Dispute Resolved",
        message: `The dispute has been resolved by admin. Decision: ${
          decision === "SELLER"
            ? "In your favour"
            : decision === "BUYER"
            ? "In favour of buyer"
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      Dispute.find(filter)
        ...
        .populate(
          "raisedBy",
          "userName profileImage isLive is_Id_verified is_Verified_Seller is_Preferred_seller is_Preferred_seller averageRatting"
        )
        .populate(
          "sellerId",
          "userName profileImage isLive is_Id_verified is_Verified_Seller is_Preferred_seller is_Preferred_seller averageRatting"
