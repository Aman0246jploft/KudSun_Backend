    ...
    try {
      ...
      for (const item of items) {
        ...
        totalShippingCharge += Number(
          product.shippingCharge ?? DEFAULT_AMOUNT.SHIPPING_CHARGE
        );
      ...
      const sellerId = Array.from(sellerIds)[0];
      const buyerProtectionFeeSetting =
        feeMap[CHARGE_TYPE.BUYER_PROTECTION_FEE];
      let buyerProtectionFee = 0;
      let buyerProtectionFeeType = PRICING_TYPE.FIXED;
      if (buyerProtectionFeeSetting) {
        buyerProtectionFeeType = buyerProtectionFeeSetting.type;
        buyerProtectionFee =
          buyerProtectionFeeType === PRICING_TYPE.PERCENTAGE
            ? (totalAmount * buyerProtectionFeeSetting.value) / 100
            : buyerProtectionFeeSetting.value;
      }
      ...
      const shippingCharge = totalShippingCharge || 0;
      const grandTotal =
        totalAmount + shippingCharge + buyerProtectionFee + tax;
...
const createBeamPaymentLink = async (paymentData) => {
  try {
    ...
    if (!merchantId || !apiKey) {
      throw new Error(
        "Missing BEAM_MERCHANT_ID or BEAM_API_KEY in environment variables"
      );
    }
...
const originalPaymentCallback = async (req, res) => {
  ...
  const schema = Joi.object({
    ...
    paymentStatus: Joi.string()
      .valid(PAYMENT_STATUS.COMPLETED, PAYMENT_STATUS.FAILED)
      .required(),
  ...
  try {
    ...
    if (
      [PAYMENT_STATUS.COMPLETED, PAYMENT_STATUS.FAILED].includes(paymentStatus)
    ) {
      await Transaction.create({
        orderId: order._id,
        userId: order.userId,
      ...
      paymentNotifications.push(
        {
          ...
          message: `Your payment of ฿${order.grandTotal.toFixed(
            2
        ...
        },
        {
          ...
          message: `Payment of ฿${order.grandTotal.toFixed(
            2
          )} has been received.`,
          redirectUrl: `/order/${order._id}`,
        }
...
const updateOrderById = async (req, res) => {
  ...
  if (error) {
    return res
      ...
      .json(apiErrorRes(CONSTANTS_MSG.VALIDATION_ERROR, error.details));
  }
  ...
  try {
    ...
    if (!existingOrder) {
      return res
        ...
        .json(apiErrorRes("Order not found"));
    }
    ...
    if (
      value.addressId &&
      value.addressId !== String(existingOrder.addressId)
    ) {
      const address = await UserAddress.findById(value.addressId).lean();
      if (!address) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST)
          .json(apiErrorRes("Invalid addressId"));
      }
    }
    ...
    return res
      ...
      .json(apiSuccessRes(updatedOrder, "Order updated successfully"));
  } catch (err) {
...
const getBoughtProducts = async (req, res) => {
  try {
    ...
    let { paymentStatus, status, keyWord, fromDate, toDate, dateFilter } =
      req.query;
      ...
      .populate([
        {
          ...
          model: "SellProduct",
          select:
            "title productImages fixedPrice status saleType auctionSettings",
        },
        {
          path: "sellerId",
          select:
    ...
    if (keyWord && keyWord.trim() !== "") {
      ...
      orders = orders.filter((order) => {
        ...
        // check if seller username matches
        const sellerMatch = order.sellerId?.userName
          ?.toLowerCase()
          .includes(lowerKey);
        return productMatch || sellerMatch;
      });
  ...
  try {
      ...
      .populate([
        {
          ...
          model: "SellProduct",
          select:
            "title productImages fixedPrice saleType auctionSettings userId deliveryType",
        },
        {
          path: "userId",
          select:
    ...
    if (keyWord && keyWord.trim() !== "") {
      ...
      orders = orders.filter((order) => {
        ...
        const userMatch = order.userId?.userName
          ?.toLowerCase()
    ...
    for (const order of orders) {
      ...
      const allLocalPickup = order.items.every(
        (item) => item.productId?.deliveryType === "local pickup"
      );
      ...
      order.items.forEach((item) => {
        if (
          item.productId?._id &&
          reviewedSet.has(item.productId._id.toString())
        ) {
...
//             const buyerUser = await User.findById(order.userId).lean();

//             if (buyerUser?.alertNotification !== false) {
  ...
  try {
    ...
    switch (newStatus) {
      case ORDER_STATUS.CONFIRM_RECEIPT:
        ...
        notificationMessage =
          "The buyer has confirmed receipt of the order. Transaction completed successfully!";
        break;
      case ORDER_STATUS.RETURNED:
        notificationTitle = "Return Requested";
        notificationMessage =
          "The buyer has requested a return for this order. Please review the return request.";
        break;
      default:
  ...
  try {
    ...
    const order = await Order.findOne({ _id: orderId, isDeleted: false })
      .populate({
        ...
        model: "SellProduct",
        select:
          "title productImages _id fixedPrice saleType auctionSettings shippingCharge deliveryType",
      })
      .populate({
        path: "userId",
        select:
    ...
    const reviews = await Promise.all(
      (order.items || []).map(async (item) => {
        ...
        const enrichedReviews = productReviews.map((review) => ({
          ...review,
          isYourReview:
            review.userId?._id?.toString() === req.user.userId.toString(),
        }));
    ...
    let isLocalPickup =
      (order.items || [])[0]?.productId?.deliveryType ===
    ...
    const statusSet = new Set(
      (statusHistory || []).map((h) => h.newStatus?.toLowerCase())
    );
    ...
    const getStepStatus = (step, index, allSteps) => {
      ...
      if (step === "review")
        return (
          reviews?.length > 0 && reviews[0].reviews?.some((r) => r.isYourReview)
        );
      return false;
    };
    ...
    const shippedStatus = statusHistory.find(
      (h) => h.newStatus?.toLowerCase() === "shipped"
    );
      ...
      .populate([
        {
          ...
          model: "SellProduct",
          select:
            "title productImages description fixedPrice saleType auctionSettings userId deliveryType",
        },
        {
          path: "userId",
          select:
...
const addrequest = async (req, res) => {
  ...
  try {
    ...
    const minSetting = withdrawalSettings.find(
      (e) => e.name === WSETTING.Minimum_Amount
    );
    const maxSetting = withdrawalSettings.find(
      (e) => e.name === WSETTING.Maximum_Amount
    );
    ...
    await session.withTransaction(async () => {
      ...
      const withDrawlSetting = feeSettings.find(
        (f) => f.name === "WITHDRAWAL_FEE"
      );
      ...
      if (user.alertNotification !== false) {
...
const trackWithdrawalRevenue = async (
  withdrawalRequest,
  withdrawalFee,
  session
) => {
...
const changeStatus = async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const withdrawRequest = await SellerWithdrawl.findById(
        withdrawRequestId
      ).session(session);
      if (!withdrawRequest) {
        ...
        if (withdrawRequest.withdrawfeeType === PRICING_TYPE.PERCENTAGE) {
          calculatedFee =
            (Number(withdrawRequest.amount) *
              Number(withdrawRequest.withdrawfee)) /
            100;
  ...
  try {
    ...
    const orders = await Order.find(filter)
      .populate([
        ...
        },
        {
          path: "disputeId",
          select:
            "disputeId status disputeType description adminReview createdAt",
        },
      ])
      .sort(sort)
      .skip((pageNo - 1) * size)
    ...
    if (paidToSeller !== undefined) {
      ...
      formattedOrders = formattedOrders.filter(
        (order) => order.sellerPayout?.isPaidToSeller === isPaid
      );
    }
    ...
    if (disputeStatus) {
      formattedOrders = formattedOrders.filter(
        (order) => order.dispute?.status === disputeStatus
      );
    }
...
const getSellerPayoutStatus = async (req, res) => {
  try {
    ...
    return apiErrorRes(
      HTTP_STATUS.INTERNAL_SERVER_ERROR,
      res,
      err.message || "Failed to get payout status"
    );
  }
};
...
const getSellerPayoutCalculation = async (req, res) => {
  try {
    ...
    if (
      disputeInfo &&
      disputeInfo.hasResolution &&
      disputeInfo.status === DISPUTE_STATUS.RESOLVED
    ) {
      ...
      if (decision === DISPUTE_DECISION.SELLER) {
        ...
        calculatedProductCost =
          originalProductCost * (sellerReceivePercent / 100);
    ...
    if (serviceChargeSetting) {
      if (serviceChargeSetting.type === PRICING_TYPE.PERCENTAGE) {
        calculatedServiceCharge =
          (calculatedProductCost * serviceChargeSetting.value) / 100;
      } else {
    ...
    calculatedNetAmount =
      calculatedProductCost - calculatedServiceCharge - calculatedTaxCharge;
    ...
    if (withdrawalFeeSetting) {
      if (withdrawalFeeSetting.type === PRICING_TYPE.PERCENTAGE) {
        withdrawalFee =
          (finalAmounts.netAmount * withdrawalFeeSetting.value) / 100;
        withdrawalFeeType = PRICING_TYPE.PERCENTAGE;
      } else {
        withdrawalFee = withdrawalFeeSetting.value;
        withdrawalFeeType = PRICING_TYPE.FIXED;
...
router.post(
  "/updateOrderStatusBySeller/:orderId",
  perApiLimiter(),
  upload.none(),
...
router.get(
  "/confirmreciptReview/:orderId",
  perApiLimiter(),
  upload.none(),
  confirmreciptReview
);
router.post(
  "/updateOrderStatusByBuyer/:orderId",
  perApiLimiter(),
  upload.none(),
...
const getProductFinancialDetails = async (req, res) => {
  try {
    ...
    const response = {
      ...
      sellerPayouts: {
        totalPaidToSeller: sellerPayouts.reduce(
          (sum, payout) => sum + (payout.netAmount || 0),
          0
        ),
        totalServiceCharges: sellerPayouts.reduce(
          (sum, payout) => sum + (parseFloat(payout.serviceCharge) || 0),
          0
        ),
        totalTaxCharges: sellerPayouts.reduce(
          (sum, payout) => sum + (parseFloat(payout.taxCharge) || 0),
          0
        ),
      ...
      withdrawals: {
        ...
        totalWithdrawalFees: withdrawals.reduce(
          (sum, w) => sum + (w.withdrawfee || 0),
          0
        ),
        withdrawalCount: withdrawals.length,
      },
      platformRevenue: {
        total: platformRevenueFromProduct.reduce(
      ...
      disputes: {
        ...
        resolved: disputes.filter((d) => d.status === DISPUTE_STATUS.RESOLVED)
          .length,
        pending: disputes.filter((d) => d.status === DISPUTE_STATUS.PENDING)
          .length,
        totalFinancialImpact: disputes
          .filter((d) => d.adminReview?.disputeAmountPercent > 0)
          .reduce((sum, d) => {
            const order = orders.find(
              (o) => o._id.toString() === d.orderId.toString()
            );
            return (
              sum +
              (order
                ? (order.grandTotal * d.adminReview.disputeAmountPercent) / 100
                : 0)
            );
          }, 0),
      },
    ...
    if (includeHistory === "true") {
      response.transactionHistory = orders.map((order) => {
        ...
        const sellerPayout = sellerPayouts.find(
          (p) => p.orderId.toString() === order._id.toString()
        );
        const withdrawal = withdrawals.find(
          (w) => w.orderId.toString() === order._id.toString()
        );
  ...
  try {
    ...
    const enhancedOrders = orders.map((order) => {
      ...
      const totalPlatformRevenue =
        buyerProtectionFee + tax + serviceCharges + taxCharges + withdrawalFees;
    ...
    const summary = {
      ...
      totalGMV: enhancedOrders.reduce(
        (sum, order) => sum + order.moneyFlow.buyerPayment.grandTotal,
        0
      ),
      totalPlatformRevenue: enhancedOrders.reduce(
        (sum, order) =>
          sum + order.moneyFlow.platformRevenue.totalPlatformRevenue,
        0
      ),
      ...
      totalDisputeImpact: enhancedOrders.reduce((sum, order) => {
        return (
          sum +
          order.moneyFlow.disputeImpact.reduce(
            (disputeSum, dispute) => disputeSum + dispute.financialImpact,
            0
          )
        );
      }, 0),
...
router.get(
  "/admin/product-financial/:productId",
  perApiLimiter(),
  upload.none(),
  getProductFinancialDetails
);
router.get(
  "/admin/money-flow",
  perApiLimiter(),
  upload.none(),
  getDetailedMoneyFlow
);

router.get(
  "/admin/payoutCalculation/:orderId",
  perApiLimiter(),
  upload.none(),
  getSellerPayoutCalculation
);
...
const cancelOrderByBuyer = async (req, res) => {
  ...
  try {
    ...
    // Find the order and validate ownership - UNCHANGED
    const order = await Order.findOne({
      _id: orderId,
      $or: [{ userId: buyerId }, { sellerId: buyerId }],
      isDeleted: false,
    }).populate("items.productId");
    ...
    // Check if order is already in a terminal status - UNCHANGED
    ...
    // Populate product data to check delivery types - UNCHANGED
    const populatedOrder = await order.populate("items.productId");
    ...
    const result = await executeWithRetry(async () => {
      // Start transaction for atomic operations - UNCHANGED
      const session = await mongoose.startSession();
      session.startTransaction();

      try {
        // Update order status and cancellation details - UNCHANGED
        const updatedOrder = await Order.findByIdAndUpdate(
          order._id,
          {
            status: ORDER_STATUS.CANCELLED,
        ...
        // Create order status history - UNCHANGED
        await OrderStatusHistory.create(
          [
            {
              orderId: order._id,
        ...
        // Reset product availability (set isSold = false) - UNCHANGED
        const productIds = order.items.map((item) => item.productId._id);
        await SellProduct.updateMany(
          {
            _id: { $in: productIds },
        ...
        // Create or get chat room for system message - UNCHANGED
        const { room } = await findOrCreateOneOnOneRoom(
          buyerId,
          order.sellerId
        );

        // Create system message for cancellation - UNCHANGED
        const systemMessage = new ChatMessage({
          chatRoom: room._id,
          messageType: "ORDER_STATUS",
          content: "Order cancelled by buyer",
        ...
        // Update chat room's last message - UNCHANGED
        await ChatRoom.findByIdAndUpdate(
          room._id,
          {
            lastMessage: systemMessage._id,
        ...
        // Emit system message to both parties - UNCHANGED
        await emitSystemMessage(
          io,
          systemMessage,
          room,
        ...
        // Send notification to seller about cancellation - UNCHANGED
        const sellerNotification = [
          {
            recipientId: order.sellerId,
            userId: buyerId,
        ...
        // Handle refund logic for paid orders - UNCHANGED
        let refundStatus = "not_required";
        let refundAmount = 0;
        ...
        if (
          ...
          console.log(
            `💰 Refund processed: ฿${refundAmount} credited to buyer wallet`
          );
          ...
          const buyerRefundNotification = [
            {
              ...
              type: NOTIFICATION_TYPES.PAYMENT,
              title: "Refund Processed",
              message: `Your refund of ฿${refundAmount.toFixed(
                2
              )} has been credited to your wallet for the cancelled order.`,
              meta: createStandardizedNotificationMeta({
                orderNumber: order._id.toString(),
                orderId: order._id.toString(),
                amount: refundAmount,
