        ...
        $or: [
          {
            $and: [
              { raterRole: "buyer" },
              {
                "reviewedUser.averageRatting": {
                  $gte: parseFloat(sellerRatingMin),
                },
              },
              {
                "reviewedUser.averageRatting": {
                  $lte: parseFloat(sellerRatingMax),
                },
              },
            ],
          },
        ...
        $or: [
          {
            $and: [
              { raterRole: "seller" },
              {
                "reviewedUser.averageBuyerRatting": {
                  $gte: parseFloat(buyerRatingMin),
                },
              },
              {
                "reviewedUser.averageBuyerRatting": {
                  $lte: parseFloat(buyerRatingMax),
                },
              },
            ],
          },
...
async function recalculateUserRatings(userId, raterRole, session) {
  try {
    if (raterRole === "buyer") {
      ...
      console.log(
        `✅ Buyer ratings recalculated for user ${userId}: ${stats.averageBuyerRating.toFixed(
          2
        )} (${stats.totalBuyerRatingCount} reviews)`
      );
    }
  } catch (error) {
