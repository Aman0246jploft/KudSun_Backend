...
const addThread = async (req, res) => {
  try {
    ...
    if (tags) {
      ...
      tagArray = raw.map((id) => id.trim?.()).filter((id) => id);
    }
    let photoUrls = [];
    ...
    if (draftMode) {
      ...
      try {
        ...
        console.error(
          "Algolia indexing failed for thread:",
          saved._id,
          algoliaError
        );
...
const updateThread = async (req, res) => {
  try {
    ...
    if (budgetFlexible === "true" || budgetFlexible === true) {
      ...
      budgetRange.min =
        min !== undefined ? Number(min) : existing.budgetRange?.min;
      budgetRange.max =
        max !== undefined ? Number(max) : existing.budgetRange?.max;
    }

    const updateData = {
      categoryId: categoryId !== undefined ? categoryId : existing.categoryId,
      subCategoryId:
        subCategoryId !== undefined ? subCategoryId : existing.subCategoryId,
      title: title !== undefined ? title : existing.title,
      description:
        description !== undefined ? description : existing.description,
      budgetFlexible:
        budgetFlexible !== undefined
    ...
    if (!draftMode) {
      try {
        ...
        console.error(
          "Algolia update failed for thread:",
          updated._id,
          algoliaError
        );
        // Don't fail the main operation if Algolia fails
  ...
  try {
    ...
    if (!draftMode) {
      try {
        ...
        console.error(
          "Algolia deletion failed for thread:",
          existing._id,
          algoliaError
        );
        // Don't fail the main operation if Algolia fails
...
const getThreadByUserId = async (req, res) => {
  try {
    ...
    const threadsWithProductCount = await Thread.aggregate([
      ...
      {
        $unwind: {
          path: "$comments.associatedProducts",
...
const addComment = async (req, res) => {
  try {
    ...
    const thread = await Thread.findById(value.thread).populate(
      "userId",
    ...
    if (thread && commenter) {
      ...
      // Case 2: Reply to a comment
        ...
        const parentComment = await ThreadComment.findById(
          value.parent
        ...
        if (parentComment && parentComment.author) {
          ...
          if (
            thread.userId._id.toString() !== req.user.userId.toString() &&
      ...
      if (productIds.length > 0) {
        try {
          ...
          if (
            ...
            const productTitles = associatedProducts
              .map((p) => p.title)
              .join(", ");
            const truncatedProducts =
              productTitles.length > 60
                ? productTitles.substring(0, 60) + "..."
                : productTitles;
      ...
      if (notifications.length > 0) {
        try {
          ...
          const filteredNotifications = notifications.filter((n) =>
            allowedRecipientIds.has(n.recipientId.toString())
          );
...
const associatedProductByThreadId = async (req, res) => {
  try {
    ...
    if (provinceId && provinceId.length === 24)
      userFilter.provinceId = toObjectId(provinceId);
    if (districtId && districtId.length === 24)
      userFilter.districtId = toObjectId(districtId);
    if (averageRatting)
      userFilter.averageRatting = { $gte: parseFloat(averageRatting) };
    ...
    const products = await SellProduct.find(filter)
      .populate({
        path: "userId",
        select:
          "userName email is_Verified_Seller is_Preferred_seller profileImage isLive is_Id_verified",
      })
      .sort({ [sortBy]: sortOrder })
      .skip((pageNo - 1) * size)
      .limit(size)
...
const getThreads = async (req, res) => {
  try {
    ...
    if (Object.keys(userFilter).length > 0) {
      ...
      if (filters.userId) {
        filters.userId = matchedUserIds.includes(filters.userId.toString())
          ? filters.userId
          : null;
      } else {
        filters.userId = { $in: matchedUserIds };
    ...
    const userIds = [
      ...new Set(
        paginatedThreads.map((t) => t.userId?._id?.toString()).filter(Boolean)
      ),
    ];

    const [followerCounts, commentCounts, likeCounts, productCounts] =
      await Promise.all([
        Follow.aggregate([
          {
            $match: {
        ...
        ThreadComment.aggregate([
          {
            $match: {
              thread: { $in: threadIds },
              associatedProducts: { $exists: true, $not: { $size: 0 } },
            },
          },
          {
            $group: {
    ...
    if (currentUserId) {
      const userLikes = await ThreadLike.find({
        ...
        .select("threadId")
        .lean();
      likedThreadSet = new Set(
        userLikes.map((like) => like.threadId.toString())
      );
    }
    ...
    const categories = await Category.find({
      "subCategories._id": {
        $in: paginatedThreads.map((t) => t.subCategoryId),
      },
    }).lean();
    ...
    await Promise.all(
      threadIds.map(async (threadId) => {
        const productIds = await getAssociatedProductIdsFromThread(
          threadId.toString()
        );
  ...
  try {
    ...
    const thread = await Thread.findOne({ _id: threadId, isDeleted: false })
      .populate({
        path: "userId",
        select:
          "userName profileImage isLive is_Id_verified is_Preferred_seller provinceId districtId",
        populate: [
    ...
    const [followerCount, commentCount, likeCount, productCount, likedByUser] =
      await Promise.all([
    ...
    const topComments = await ThreadComment.aggregate([
      ...
      },
      {
        $addFields: {
          totalReplies: {
            $ifNull: [{ $arrayElemAt: ["$replyCount.totalReplies", 0] }, 0],
          },
        },
      },
      {
    ...
    let commentProductIds =
      allAssociatedProductIdsAgg[0]?.productIds?.map((id) => id.toString()) ||
      [];
    ...
    const associatedProductsFull = await SellProduct.aggregate([
...

      {
        $addFields: {
          totalBids: {
            $cond: [{ $eq: ["$saleType", "auction"] }, { $size: "$bids" }, 0],
          },
        },
      },
        ...
        $project: {
          ...
          productCount: {
            $ifNull: [
              {
                $arrayElemAt: [
                  "$commentProductStats.totalAssociatedProducts",
                  0,
                ],
              },
              0,
...
const getFollowedUsersThreads = async (req, res) => {
  try {
    ...
    const threads = await Thread.find(filters)
      .populate(
        "userId",
        "userName profileImage isLive is_Id_verified is_Preferred_seller"
      )
      .sort(sortStage)
      .skip((page - 1) * limit)
      .limit(limit)
    ...
    const threadIds = threads.map((t) => t?._id);
    const userIds = [
      ...new Set(threads.map((t) => t.userId?._id?.toString()).filter(Boolean)),
    ];
          ...
          pipeline: [
            ...
            },
            {
              $addFields: {
                totalComments: {
                  $ifNull: [
                    { $arrayElemAt: ["$commentsCountInfo.totalComments", 0] },
                    0,
                  ],
                },
                totalLikes: {
                ...
                isLiked: {
                  $gt: [
                    {
                      $ifNull: [
                        { $arrayElemAt: ["$likesInfo.likedByCurrentUser", 0] },
                        0,
                      ],
                    },
                    0,
    ...
    await Promise.all(
      users.map(async (user) => {
        if (user.latestThread?._id) {
          const validProducts = await getAssociatedProductIdsFromThread(
            user.latestThread._id
          );
  ...
  try {
    ...
    const [products, totalCount] = await Promise.all([
      SellProduct.find(matchConditions)
        .select(
          "_id title description fixedPrice saleType productImages createdAt"
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(parseInt(size))
...
router.post(
  "/update-all-thread-trending",
  perApiLimiter(),
  updateAllThreadTrending
);
...
router.get(
  "/selectProductForAssociation",
  perApiLimiter(),
  upload.none(),
  selectProductForAssociation
);
router.get(
  "/associatedProductByThreadId/:threadId",
  perApiLimiter(),
  upload.none(),
  associatedProductByThreadId
);
