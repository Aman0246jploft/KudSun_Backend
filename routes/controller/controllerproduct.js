...
async function ensureParameterAndValue(
  categoryId,
  subCategoryId,
  key,
  value,
  ...
  let parameter = subCat.parameters.find(
    (p) => p.key === key.toLowerCase().trim()
  );
  ...
  if (!parameter) {
    ...
    await category.save();
    parameter = subCat.parameters.find(
      (p) => p.key === key.toLowerCase().trim()
    ); // re-fetch
  } else {
    const existingValue = parameter.values.find(
      (v) => v.value === value.toLowerCase().trim()
    );
    if (!existingValue) {
  ...
  const paramRef = subCat.parameters.find(
    (p) => p.key === key.toLowerCase().trim()
  );
  const valueRef = paramRef.values.find(
    (v) => v.value === value.toLowerCase().trim()
  );
  ...
  try {
    ...
    try {
      if (req.body.specifics) {
        const parsed =
          typeof req.body.specifics === "string"
    ...
    if (saleType === SALE_TYPE.AUCTION) {
      ...
      if (userEndDate && endTime) {
        ...
        biddingEndsAtDateTime = DateTime.fromISO(dateTimeString, {
          zone: timeZone,
        });
      ...
      const testDateTime = DateTime.fromISO("2025-07-12T18:44:00", {
        zone: "Asia/Kolkata",
      });

      auctionSettings.biddingEndsAt = new Date(
        biddingEndsAtDateTime.toUTC().toISO()
      );
      auctionSettings.isBiddingOpen =
        DateTime.now().setZone("UTC") < biddingEndsAtDateTime.toUTC();
    ...
    const productData = {
      ...
      originPrice,
      auctionSettings:
        saleType === SALE_TYPE.AUCTION ? auctionSettings : undefined,
      deliveryType,
      shippingCharge:
        deliveryType === DeliveryType.CHARGE_SHIPPING
          ? shippingCharge
    ...
    if (isDraft === "true" || isDraft === true) {
      ...
      try {
        ...
        console.error(
          "Algolia indexing failed for product:",
          savedProduct._id,
          algoliaError
        );
      ...
      if (draftId) {
        try {
          ...
          if (deletedDraft) {
            ...
            console.warn(
              `Draft product ${draftId} not found or user doesn't own it`
            );
          }
        } catch (draftDeleteError) {
          console.error(
            "Error deleting draft after publishing:",
            draftDeleteError
          );
...
const updateSellerProduct = async (req, res) => {
  try {
    ...
    const isDraftUpdate =
      req?.body?.isDraft === "true" || req?.body?.isDraft === true || false;
    ...
    if (!isDraftUpdate) {
      ...
      if (saleType === SALE_TYPE.AUCTION) {
        ...
        if (endDate && endTime) {
          biddingEndsAtDateTime = DateTime.fromISO(`${endDate}T${endTime}`, {
        ...
        auctionSettings.biddingEndsAt = biddingEndsAtDateTime
          .toUTC()
          .toJSDate();
        auctionSettings.isBiddingOpen =
          DateTime.now().setZone("UTC") < biddingEndsAtDateTime.toUTC();
    ...
    if (imageArray) {
      ...
      const cleanBodyUrls = bodyUrls.filter(
        (url) => typeof url === "string" && url.trim() !== ""
      );
      ...
      const imagesToDelete = photoUrls.filter(
        (url) => !cleanBodyUrls.includes(url)
      );
    ...
    if (originPriceView !== undefined)
      existingProduct.originPriceView =
        originPriceView === "true" || originPriceView === true;
    ...
    if (shippingCharge !== undefined)
      existingProduct.shippingCharge = shippingCharge;
    if (specifics !== undefined) existingProduct.specifics = specifics;
    if (tags !== undefined)
      existingProduct.tags = Array.isArray(tags) ? tags : [tags];
    if (auctionSettings !== undefined)
      existingProduct.auctionSettings = auctionSettings;
    if (isDisable !== undefined)
      existingProduct.isDisable = isDisable === "true" || isDisable === true;
    ...
    if (!isDraftUpdate) {
      try {
        ...
        console.error(
          "Algolia update failed for product:",
          updatedProduct._id,
          algoliaError
        );
        // Don't fail the main operation if Algolia fails
...
const toggleProductDisable = async (req, res) => {
  try {
    ...
    const productPrice =
      product.saleType === SALE_TYPE.AUCTION
    ...
    if (notifications.length > 0) {
      try {
        ...
        const allowedIdsSet = new Set(
          allowedRecipients.map((u) => u._id.toString())
        );
        ...
        if (filteredNotifications.length > 0) {
          ...
          console.log(
            "⚠️ Product owner has disabled activity notifications. Skipping dispatch."
          );
        }
      } catch (notificationError) {
        console.error(
          "❌ Failed to send toggle product notifications:",
          notificationError
        );
      }
    }
...
const showNormalProducts = async (req, res) => {
  try {
    ...
    if (deliveryFilter === "free") {
      filter.deliveryType = {
        $in: [DeliveryType.FREE_SHIPPING, DeliveryType.LOCAL_PICKUP],
      };
    ...
    if (specifics) {
      const parsedSpecifics = Array.isArray(specifics)
        ? specifics
        : [specifics];
      filter["specifics.valueId"] = {
        $all: parsedSpecifics.map((id) => toObjectId(id)),
      };
    }
    ...
    if (sortBy === "commentCount") {
      ...
      await SellProduct.populate(products, [
        { path: "categoryId", select: "name" },
        {
          path: "userId",
          select:
            "userName profileImage averageRatting is_Id_verified isLive is_Preferred_seller",
        },
      ]);
      ...
      if (products.length) {
        const categoryIds = [
          ...new Set(products.map((p) => p.categoryId?._id?.toString())),
        ];
    ...
    const [products, total] = await Promise.all([
      SellProduct.find(filter)
        ...
        .limit(limit)
        .select(
          "title fixedPrice saleType shippingCharge deliveryType isTrending isDisable productImages condition subCategoryId isSold userId  tags originPriceView originPrice description specifics createdAt"
        )
...
const showAuctionProducts = async (req, res) => {
  try {
    ...
    if (req.query.spec) {
      const specParams = Array.isArray(req.query.spec)
        ? req.query.spec
        : [req.query.spec];
      const specFilters = specParams
        .map((pair) => {
    ...
    if (sortBy === "commentCount") {
      ...
      await SellProduct.populate(products, [
        { path: "categoryId", select: "name" },
        {
          path: "userId",
          select:
            "userName profileImage is_Id_verified isLive is_Preferred_seller",
        },
      ]);
      ...
      for (const product of products) {
        const utcEnd = DateTime.fromJSDate(
          product.auctionSettings.biddingEndsAt,
          { zone: "utc" }
        );
        const utcDate = DateTime.fromJSDate(
          product.auctionSettings.biddingEndsAt,
          { zone: "utc" }
        );
        const localDate = utcDate.setZone(
          product.auctionSettings.timeZone || "Asia/Kolkata"
        );
        ...
        const localNow = new Date(
          nowTimestamp.getTime() - offsetMinutes * 60 * 1000
        );
        const endTime = new Date(
          product.auctionSettings.biddingEndsAt
        ).getTime();
        product.timeRemaining =
          timeLeftMs > 0 ? formatTimeRemaining(timeLeftMs) : 0;
      ...
      if (req.user && req.user.userId) {
        ...
        likedProductIds = new Set(
          likes.map((like) => like.productId.toString())
        );
      }
    ...
    if (sortBy === "bidCount") {
      ...
      for (const product of products) {
        ...
        const localDate = utcEnd.setZone(
          product.auctionSettings.timeZone || "Asia/Kolkata"
        );
        const utcNow = DateTime.utc();
        const timeLeftMs = utcEnd.diff(utcNow).toMillis();
    ...
    const [products, total] = await Promise.all([
      SellProduct.find(filter)
        ...
        .limit(limit)
        .select(
          "title productImages isSold condition isDisable subCategoryId auctionSettings tags description specifics createdAt"
        )
        .populate("categoryId", "name")
        .populate(
          "userId",
    ...
    const bidsCounts = await Bid.aggregate([
      { $match: { productId: { $in: productIds } } },
      {
        $group: {
          _id: "$productId",
          totalBidsPlaced: { $sum: 1 },
          highestBidAmount: { $max: "$amount" },
        },
      },
    ]);
    ...
    products.forEach((product) => {
      ...
      const utcNow = DateTime.utc();
      const timeLeftMs = utcEnd.diff(utcNow).toMillis();
      const bidData = bidsCountMap[product._id.toString()] || {
        totalBidsPlaced: 0,
        highestBidAmount: 0,
      };
  ...
  try {
    ...
    const [products, total] = await Promise.all([
      SellProduct.find(filter)
        ...
        .limit(limit)
        .select(
          "title productImages auctionSettings.reservePrice condition tags description createdAt auctionSettings.biddingEndsAt"
        )
        .populate("categoryId", "name")
...
const fetchCombinedProducts = async (req, res) => {
  try {
    ...
    if (specifics) {
      ...
      limitedFilter["specifics.valueId"] = {
        $all: parsed.map((id) => toObjectId(id)),
      };
    }
    ...
    if (limitedProductsRaw.length) {
      ...
      const bidsMap = bidAgg.reduce(
        (a, c) => ({ ...a, [c._id]: c.totalBidsPlaced }),
        {}
      );
    ...
    saleTypeFilters = saleTypeFilters.map((filter) => {
      ...
      if (specifics) {
        const parsedSpecifics = Array.isArray(specifics)
          ? specifics
          : [specifics];
        updatedFilter["specifics.valueId"] = {
          $all: parsedSpecifics.map((id) => toObjectId(id)),
        };
      }
    ...
    let enhancedProducts = allProducts.map((product) => {
      const enhancedProduct = {
        ...product,
        productType:
          product.saleType === SALE_TYPE.FIXED ? "normal" : "auction",
      };

      // Add bid count for auction products
      if (product.saleType === SALE_TYPE.AUCTION) {
        enhancedProduct.totalBidsPlaced =
          bidsCountMap[product._id.toString()] || 0;
        ...
        if (product.auctionSettings) {
          ...
          enhancedProduct.timeRemainingFormatted =
      ...
      enhancedProduct.sortPrice =
        product.saleType === SALE_TYPE.FIXED
          ? product.fixedPrice
    ...
    enhancedProducts.sort((a, b) => {
      switch (sortBy) {
        ...
        case "endingSoon":
          ...
          if (
            a.saleType === SALE_TYPE.AUCTION &&
            b.saleType === SALE_TYPE.AUCTION
          ) {
            return (
              new Date(a.auctionSettings?.biddingEndsAt || 0) -
              new Date(b.auctionSettings?.biddingEndsAt || 0)
            );
          } else if (
            a.saleType === SALE_TYPE.AUCTION &&
            a.auctionStatus?.isActive
          ) {
            return -1; // Active auctions first
          } else if (
            b.saleType === SALE_TYPE.AUCTION &&
            b.auctionStatus?.isActive
          ) {
            return 1; // Active auctions first
          }
    ...
    const finalProducts = paginatedProducts.map((p) => {
      ...
      return {
        ...
        ...(p.timeRemainingFormatted && {
          timeRemainingFormatted: p.timeRemainingFormatted,
        }),
        ...(typeof p.totalBidsPlaced !== "undefined" && {
...
const fetchUserProducts = async (req, res) => {
  try {
    ...
    const isSelfProfile =
      (userId && requesterId && userId === requesterId) || false;
    ...
    if (
      ...
      if (req.query.provinceId && req.query.provinceId !== "") {
        userFilter.provinceId = toObjectId(req.query.provinceId);
      }
      ...
      if (req.query.averageRatting && req.query.averageRatting !== "") {
        const avgRating = parseFloat(req.query.averageRatting);
        if (!isNaN(avgRating)) {
          userFilter.averageRatting = { $gte: avgRating };
        }
  ...
  try {
    ...
    const viewHistory = await SearchHistory.find({
      ...
      .populate({
        path: "productId",
        select:
          "title productImages fixedPrice saleType condition isSold userId",
        populate: {
          path: "userId",
...
const getProduct = async (req, res) => {
  try {
    ...
    const user = await User.findById(product.userId)
      .select(
        "userName profileImage is_Id_verified is_Preferred_seller isLive averageRatting "
      )
      .lean();
    ...
    if (product.saleType === SALE_TYPE.AUCTION) {
      const allBids = await Bid.find({ productId: id })
        .populate({
          path: "userId",
      ...
      const currentHighestBid = allBids.reduce(
    ...
    const recommendedProducts = await SellProduct.find({
      ...
      .limit(10)
      .select(
        "fixedPrice auctionSettings productImages isSold title saleType userId"
      ) // include userId to populate it
      .populate({
        path: "userId",
        select:
...
const addComment = async (req, res) => {
  try {
    ...
    const product = await SellProduct.findById(value.product).populate(
      "userId",
      "userName profileImage"
    );
    const commenter = await User.findById(req.user?.userId).select(
      "userName profileImage"
    );
    ...
    if (product && commenter) {
      ...
      // Case 2: Reply to a comment
        ...
        const parentComment = await ProductComment.findById(
          value.parent
        ...
        if (parentComment && parentComment.author) {
          ...
          if (
            parentComment.author._id.toString() !== req.user.userId.toString()
          ) {
            notifications.push({
      ...
      if (productIds.length > 0) {
        try {
          ...
          for (const assocProduct of associatedProducts) {
            ...
            if (
              assocProduct.userId._id.toString() !== req.user.userId.toString()
            ) {
              ...
              const assocProductPrice =
                assocProduct.saleType === "auction"
          ...
          console.error(
            "❌ Failed to fetch associated products for notifications:",
            productError
          );
      ...
      if (notifications.length > 0) {
        try {
          ...
          if (filteredNotifications.length > 0) {
            await saveNotification(filteredNotifications);
            console.log(
              `✅ ${filteredNotifications.length} notification(s) sent for product comment and associations`
            );
          } else {
            console.log(
              "⚠️ All users have disabled activity notifications. Skipping notification dispatch."
            );
          }
        } catch (notificationError) {
          console.error(
            "❌ Failed to send product comment notifications:",
            notificationError
          );
        }
      }
...
const deleteProduct = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        "Algolia deletion failed for product:",
        product._id,
        algoliaError
      );
      // Don't fail the main operation if Algolia fails
...
const deleteProductDraft = async (req, res) => {
  try {
    ...
    // Find product and check existence
    const product = await SellProductDraft.findOne({
      _id: id,
      isDeleted: false,
    });
    if (!product) {
  ...
  try {
    ...
    if (
      ...
      if (minPrice !== undefined && minPrice !== "")
        filter.fixedPrice.$gte = Number(minPrice);
      if (maxPrice !== undefined && maxPrice !== "")
        filter.fixedPrice.$lte = Number(maxPrice);
    }
    ...
    const products = await Model.find(filter)
      .populate({
        ...
        match: {
          ...
          isDisable: false,
          ...(minSellerRating && {
            averageRatting: { $gte: Number(minSellerRating) },
          }),
