    ...
    cron.schedule("* * * * *", async () => {
      ...
      try {
        ...
        for (const product of expiredAuctions) {
          ...
          if (highestBid) {
            const userAddressInfo = await UserAddress.findOne({
              userId: toObjectId(highestBid.userId),
              isActive: true,
            });
            ...
            const grandTotal =
              highestBid.amount + shippingCharge + buyerProtectionFee + tax;
            ...
            if (!existingOrder) {
              ...
              console.log(
                `✅ Order created for product: ${product._id} (Bid: ${highestBid.amount})`
              );
              ...
              console.log(
                `⚠️ Order already exists for product: ${product._id}`
              );
            }
          } else {
            console.log(
              `❌ No valid bid for product: ${product._id} (reserve not met)`
            );
          }
