...
mongoose
  ...
  .then(() => {
    ...
    console.log(
      `⏱️ Processing orders older than: ${PROCESSING_DAY_LIMIT} days`
    );
    ...
    cron.schedule(CRON_SCHEDULE, async () => {
      ...
      try {
        ...
        const shippedStats = await updateShippedToDelivered(
          cutoffDate,
          session
        );
        ...
        const completedStats = await updateDeliveredToCompleted(
          cutoffDate,
          session
        );
        ...
        const confirmReceiptStats = await updateConfirmReceiptToCompleted(
          now.clone().subtract(CONFIRM_RECEIPT_TO_COMPLETED_DAY_LIMIT, "days"),
          session
        );
        processedStats.confirmReceiptToCompleted = confirmReceiptStats.updated;
        processedStats.paymentsProcessed +=
          confirmReceiptStats.paymentsProcessed;
        ...
        // Update global stats
        cronStats.ordersProcessed +=
          processedStats.shippedToDelivered +
          processedStats.deliveredToCompleted +
          processedStats.confirmReceiptToCompleted +
          processedStats.disputedToCompleted;
  ...
  .catch((error) => {
    console.error(
      "❌ MongoDB connection failed for Order Status Update Cron:",
      error
    );
  });
...
async function updateShippedToDelivered(cutoffDate, session) {
  try {
    ...
    for (const order of shippedOrders) {
      try {
        ...
        if (shippedDate.isBefore(cutoffDate)) {
          console.log(
            `📅 Order ${order._id} shipped on ${shippedDate.format(
              "YYYY-MM-DD"
          ...
          if (activeDispute) {
            console.log(
              `⚠️ Order ${order._id} has active dispute, skipping auto-delivery`
            );
            continue;
          }
    ...
    console.log(
      `✅ SHIPPED → DELIVERED: ${stats.updated} orders updated, ${stats.errors.length} errors`
    );
    return stats;
  } catch (error) {
...
async function updateDeliveredToCompleted(cutoffDate, session) {
  try {
    ...
    for (const order of deliveredOrders) {
      try {
        ...
        if (deliveredDate.isBefore(cutoffDate)) {
          console.log(
            `📅 Order ${order._id} delivered on ${deliveredDate.format(
              "YYYY-MM-DD"
            )}, checking for disputes...`
          );

          // Check for disputes and validate dispute resolution
          const disputeInfo = await validateAndProcessDispute(
            order._id,
            session
          );
          ...
          const validationResult = await validateOrderForCompletion(
            order,
            session
          );
          ...
          console.log(
            `🎉 Completing order ${order._id} and processing seller payment...`
          );
          ...
          const updatedOrder = await Order.findOneAndUpdate(
            ...
            .populate("userId")          // populate buyer
            .populate("sellerId")        // populate seller
            .populate("addressId")       // populate address
            .populate("items.productId") // populate products inside items
            .exec();

          if (!updated || updated.status !== ORDER_STATUS.COMPLETED) {
            stats.errors.push(
              `Order ${order._id}: status update to COMPLETED failed`
            );
            continue;
          }
          ...
          const paymentResult = await processSellerPaymentEnhanced(
            order,
            session,
          ...
          if (paymentResult.success) {
            ...
            stats.errors.push(
              `Payment processing failed for order ${order._id}: ${paymentResult.error}`
            );
          }
    ...
    console.log(
      `✅ DELIVERED → COMPLETED: ${stats.updated} orders completed, ${stats.paymentsProcessed} payments processed, ${stats.disputesHandled} disputes handled, ${stats.errors.length} errors`
    );
    return stats;
  } catch (error) {
...
async function updateConfirmReceiptToCompleted(cutoffDate, session) {
  try {
    ...
    console.log(
      `📦 Found ${confirmReceiptOrders.length} confirm receipt orders to check`
    );
    ...
    for (const order of confirmReceiptOrders) {
      try {
        ...
        if (!confirmReceiptHistory) {
          stats.errors.push(
            `No confirm receipt history found for order ${order._id}`
          );
          continue;
        }
    ...
    console.log(
      `✅ CONFIRM_RECEIPT → COMPLETED: ${stats.updated} orders completed, ${stats.paymentsProcessed} payments processed, ${stats.disputesHandled} disputes handled, ${stats.errors.length} errors`
    );
    return stats;
  } catch (error) {
  ...
  try {
    ...
    for (const order of disputedOrders) {
      try {
        ...
        if (!dispute) {
          stats.errors.push(
            `Order ${order._id} is DISPUTE but no dispute record found`
          );
          continue;
        }
        ...
        if (dispute.status !== DISPUTE_STATUS.RESOLVED) {
          console.log(
            `⏳ Order ${order._id} dispute still ${dispute.status}, skipping...`
          );
          continue;
        }
        ...
        if (!dispute.adminReview || !dispute.adminReview.decision) {
          stats.errors.push(
            `Order ${order._id} dispute resolved but missing admin review decision`
          );
          continue;
        }
        ...
        if (
          decision === DISPUTE_DECISION.BUYER &&
          (disputeAmountPercent < 0 || disputeAmountPercent > 100)
        ) {
          stats.errors.push(
            `Order ${order._id} has invalid dispute amount percentage: ${disputeAmountPercent}%`
          );
          continue;
        }

        console.log(
          `✅ Order ${order._id} dispute resolved - Decision: ${decision}, Amount%: ${disputeAmountPercent}%`
        );
        ...
        // Proceed with completion
        console.log(
          `🎉 Completing disputed order ${order._id} and processing seller payment...`
        );
        ...
        if (paymentResult.success) {
          ...
          stats.errors.push(
            `Payment processing failed for disputed order ${order._id}: ${paymentResult.error}`
          );
        }
    ...
    console.log(
      `✅ DISPUTE → COMPLETED: ${stats.updated} orders completed, ${stats.paymentsProcessed} payments processed, ${stats.disputesHandled} disputes handled, ${stats.errors.length} errors`
    );
    return stats;
  } catch (error) {
...
async function validateAndProcessDispute(orderId, session) {
  try {
    ...
    if (resolvedDispute) {
      ...
      // Validate dispute amount percentage
      const { decision, disputeAmountPercent = 0 } =
        resolvedDispute.adminReview;
      ...
      console.log(
        `✅ Order ${orderId} had resolved dispute - Decision: ${disputeData.decision}, Amount%: ${disputeData.disputeAmountPercent}%`
      );
 ...
 */
async function processSellerPaymentEnhanced(
  order,
  session,
  disputeInfo = null
) {
  try {
    console.log(
      `💰 Processing payment for seller ${order.sellerId} for order ${order._id}`
    );
    ...
    if (disputeInfo) {
      ...
      if (decision === DISPUTE_DECISION.SELLER) {
        ...
        if (disputeAmountPercent < 0 || disputeAmountPercent > 100) {
          throw new Error(
            `Invalid dispute amount percentage: ${disputeAmountPercent}%`
          );
        }
        adjustedProductCost =
      ...
      console.log(
        `⚖️ Dispute adjustment: Original ฿${originalProductCost} → Adjusted ฿${adjustedProductCost.toFixed(
          2
        )} (${disputeAdjustmentNote})`
      );
    }
    ...
    if (netAmount < 0) {
      throw new Error(
        `Calculated net amount is negative: ฿${netAmount.toFixed(2)}`
      );
    }

    console.log(
      `💳 Payment calculation: Original: ฿${originalProductCost}, Adjusted: ฿${adjustedProductCost.toFixed(
        2
      )}, Service: ฿${serviceCharge.toFixed(2)}, Tax: ฿${taxAmount.toFixed(
        2
    ...
    console.log(
      `✅ Seller payment processed: ฿${netAmount.toFixed(
        2
      )} credited to wallet${disputeInfo ? " (dispute-adjusted)" : ""}`
    );
 ...
 */
async function trackCompletionRevenue(
  order,
  serviceCharge,
  taxAmount,
  serviceType,
  ...
  try {
    ...
    if (revenueEntries.length > 0) {
      ...
      console.log(
        `📊 Platform revenue tracked: ${revenueEntries.length} entries`
      );
    }
  } catch (error) {
 ...
 */
async function sendDisputeResolutionCompletionNotifications(
  order,
  disputeInfo
) {
  try {
    console.log(
      `📧 Sending dispute resolution completion notifications for order ${order._id}`
    );
    ...
    await saveNotification(notifications);
    console.log(
      `✅ Dispute resolution completion notifications sent for order ${order._id}`
    );
  } catch (error) {
    console.error(
      "❌ Error sending dispute resolution completion notifications:",
      error
    );
  }
}
...
async function sendCronSummaryNotification(stats, duration) {
  try {
    ...
    console.log(
      "📧 Sending cron summary notification (placeholder for email service)"
    );
...
async function sendCronErrorNotification(error) {
  try {
    console.log(
      "🚨 Sending cron error notification (placeholder for email service)"
    );
    ...
    console.error(
      "❌ Error sending cron error notification:",
      notificationError
    );
  }
}
      ...
      try {
        ...
        res.json({
          ...
          stats: {
            ...
            paymentsProcessed:
              completedStats.paymentsProcessed +
              confirmReceiptStats.paymentsProcessed +
              disputedStats.paymentsProcessed,
            disputesHandled:
              completedStats.disputesHandled +
              confirmReceiptStats.disputesHandled +
              disputedStats.disputesHandled,
            errors: [
              ...shippedStats.errors,
              ...completedStats.errors,
              ...confirmReceiptStats.errors,
              ...disputedStats.errors,
            ],
          },
        });
      } catch (error) {
...
console.log(
  "📅 Order Status Update Cron Job initialized with enhanced monitoring and dispute handling."
);
console.log(
  `⚙️ Configuration: ${PROCESSING_DAY_LIMIT} days processing limit, Schedule: ${CRON_SCHEDULE}`
);
console.log("🔍 Health check available at: /cron/health/order-status-update");
console.log(
  "🧪 Manual trigger available at: POST /cron/trigger/order-status-update"
);
