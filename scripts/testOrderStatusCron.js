...
const options = {
    ...
    days: parseInt(args.find(arg => arg.startsWith('--days='))?.split('=')[1]) || parseInt(process.env.DAY) || 3,
...
async function testShippedToDelivered(cutoffDate) {
    try {
        const shippedOrders = await Order.find({
            ...
            isDeleted: false,
            isDisable: false
        }).populate('sellerId userId', 'userName email').limit(options.limit);
        ...
        for (const order of shippedOrders) {
            try {
                ...
                if (options.verbose) {
                    console.log(`   Order ${order._id}: shipped ${daysSinceShipped} days ago`);
                }
                ...
                if (shippedDate.isBefore(cutoffDate)) {
                    ...
                    if (activeDispute) {
                        testStats.warnings.push(`Order ${order._id} has active dispute, skipping`);
                        continue;
                    }
    ...
    try {
        ...
        for (const order of deliveredOrders) {
            try {
                ...
                if (options.verbose) {
                    console.log(`   Order ${order._id}: delivered ${daysSinceDelivered} days ago`);
                }
                ...
                if (deliveredDate.isBefore(cutoffDate)) {
                    ...
                    if (disputeInfo.shouldSkip) {
                        testStats.warnings.push(`Order ${order._id}: ${disputeInfo.reason}`);
                        continue;
                    }
                    ...
                    console.log(`   ✅ Would complete order ${order._id} and process payment`);
                    if (disputeInfo.disputeData) {
                        console.log(`      - Dispute resolved: ${disputeInfo.disputeData.decision} favor`);
                    ...
                    if (!options.dryRun) {
                        ...
                        const paymentResult = await calculateSellerPayment(order, disputeInfo.disputeData);
                        if (paymentResult.success) {
                            testStats.paymentsProcessed++;
                            console.log(`      - Payment processed: ฿${paymentResult.netAmount.toFixed(2)}`);
                        } else {
                            testStats.errors.push(`Payment calculation failed for order ${order._id}: ${paymentResult.error}`);
                        }
                    }
                }
            } catch (orderError) {
        ...
        testStats.errors.push(`DELIVERED → COMPLETED test failed: ${error.message}`);
    }
}
    ...
    try {
        ...
        if (disputeInfo) {
            if (disputeInfo.decision === DISPUTE_DECISION.BUYER) {
                adjustedProductCost = originalProductCost * ((100 - disputeInfo.disputeAmountPercent) / 100);
            }
        }
        ...
        if (options.verbose) {
            console.log(`      Payment breakdown: Original: ฿${originalProductCost}, Adjusted: ฿${adjustedProductCost.toFixed(2)}, Net: ฿${netAmount.toFixed(2)}`);
        }
...
function printTestSummary() {
    ...
    console.log('');
    console.log(success ? '✅ Test completed successfully!' : '❌ Test completed with errors!');
}
