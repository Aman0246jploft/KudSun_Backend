...
const notificationTemplates = {
  ...
  chat: {
    ...
    title: "New Message 💬",
    message:
      "You have received a new message from a buyer interested in your product.",
    meta: {
...
async function validateUser(userId) {
  try {
    ...
    const user = await User.findById(userId).select(
      "userName email fcmToken notification"
    );
    if (!user) {
    ...
    log.info(
      `Notifications enabled: ${
        user.notification !== false ? "✅ Yes" : "❌ No"
      }`
    );
...
async function testFirebasePushNotification(user, notificationData) {
  try {
    ...
    if (!user.fcmToken) {
      log.warning(
        "No FCM token available for this user. Push notification will be skipped."
      );
      return false;
    }
...
async function testNotificationService(userId, notificationData) {
  try {
    ...
    if (result && result.status === SUCCESS) {
      ...
      log.info(
        "The notification will be processed by the background queue worker."
      );
      return true;
    } else {
      log.error(
        `Notification service failed. Full result: ${JSON.stringify(result)}`
      );
      return false;
    }
  } catch (error) {
...
async function runNotificationTest(userId, testType = "user") {
  try {
    ...
    const pushSuccess = await testFirebasePushNotification(
      user,
      notificationData
    );

    // Test 2: Notification Service (Queue)
    const serviceSuccess = await testNotificationService(
      userId,
      notificationData
    );

    // Test 3: Direct Database Insertion
    const dbNotification = await testDirectDatabaseInsertion(
      userId,
      notificationData
    );
...
async function main() {
  try {
    ...
    if (args.length === 0) {
      ...
      log.info(
        "Example: node scripts/testNotification.js 507f1f77bcf86cd799439011 user"
      );
      showAvailableTypes();
      process.exit(1);
    }
